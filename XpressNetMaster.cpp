...
XpressNetMasterClass::XpressNetMasterClass()
{
    ...
    for (byte b = 0; b < XNetBufferSize; b++)
    { // clear send buffer
...
void XpressNetMasterClass::setup(uint8_t FStufen, uint8_t XNetPortRX, uint8_t XNetPortTX,
                                 uint8_t XControl, bool XnModeAuto) // Initialisierung Serial
#else
void XpressNetMasterClass::setup(uint8_t FStufen, uint8_t XControl,
                                 bool XnModeAuto) // Initialisierung Serial
#endif
{
    Fahrstufe = FStufen;
...
#    endif
...
#elif defined(ESP8266) || defined(ESP32)
    XNetSwSerial.begin(62500, SWSERIAL_8S1, XNetPortRX, XNetPortTX, false,
                       95); // One Wire Half Duplex Serial, parity mode SPACE
    if (!XNetSwSerial)
    ...
    /* At high bitrates (115200bps) send bit timing can be improved at the expense of blocking
    concurrent full duplex receives, with the EspSoftwareSerial::UART::enableIntTx(false) function
    call.*/
...
{ // work on received data
    ...
    case 0x21:
        if (XNetSlaveMode == 0x00)
        {
            ...
            { // Request for Service Mode results
                ...
                {
                    uint8_t sendStatus[] = {
                      DirectedOps, 0x63, 0x14, lowByte(XNetCVAdr),
                      XNetCVvalue, 0x00}; // Service Mode response for Direct CV mode
                    getXOR(sendStatus, 6);
    ...
    case 0x22: // Start Programming
        ...
        if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x14)
        { // Paged Mode read request (Paged Mode)
        }
        if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x15)
        {                    // Direct Mode CV read request (CV mode)
            XNetCVAdr = 0;   // no CV read
            XNetCVvalue = 0; // no CV value
            if (notifyXNetDirectReadCV)
                notifyXNetDirectReadCV(XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2]
    ...
    case 0x23:
        ...
        if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x16)
        {                    // Direct Mode CV write request (CV mode)
            XNetCVAdr = 0;   // no CV read
            XNetCVvalue = 0; // no CV value
            if (notifyXNetDirectCV)
                notifyXNetDirectCV(XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] - 1,
    ...
    { // POM CV write MultiMaus
        if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x30)
        {
            uint16_t Adr = ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] & 0x3F) << 8)
                           + XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata3];
            uint16_t CVAdr = ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata4] & B11) << 8)
                             + XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata5];
            if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata4] & 0xFC) == 0xEC)
            { // set byte
                if (notifyXNetPOMwriteByte)
                    notifyXNetPOMwriteByte(Adr, CVAdr,
                                           XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata6]);
            }
            if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata4] & 0xFC) == 0xE8)
            { // set bit
                if (notifyXNetPOMwriteBit)
                    notifyXNetPOMwriteBit(
                      Adr, CVAdr, (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata6] & 0x0F));
            }
        }
        break;
    }
        ...
        { // Central Station broadcast data
            if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader] == 0x61)
            {
                if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x01)
                    && (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] == 0x60))
                {
                    // Normal Operation Resumed
                    Railpower = csNormal;
                    if (notifyXNetPower)
                ...
                else if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x00)
                         && (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] == 0x61))
                {
                    // Track power off
                    Railpower = csTrackVoltageOff;
                    if (notifyXNetPower)
                ...
                else if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x02)
                         && (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] == 0x63))
                {
                    // Service Mode Entry
            ...
            {
                if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0x00)
                    && (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] == 0x81))
                {
                    // Emergency Stop
                    Railpower = csEmergencyStop;
                    if (notifyXNetPower)
            ...
            else if ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader] & 0xF0) == 0x40)
            {
                ...
                if (notifyXNetFeedback)
                {
                    ...
                    {
                        notifyXNetFeedback(
                          (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader + (i * 2) - 1] << 2)
                            | ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader + (i * 2)]
                                & B110)
                               >> 1),
                          XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader + (i * 2)]);
                        // XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2] = 0000 ABBP
            ...
            else if (XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader] == 0x05
                     && XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata1] == 0xF1)
            {
        ...
        { // Central Station send data ...
                ...
                { // save Loco and KENNUNG
                    if (notifyXNetLocoFuncX)
                    {
                        notifyXNetLocoFuncX(SlaveRequestLocoFkt, 0x04,
                                            XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata2]);
                        notifyXNetLocoFuncX(SlaveRequestLocoFkt, 0x05,
                                            XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata3]);
                    }
            ...
            case 0xE4: // Antwort der abgefragen Lok
                ...
                { // save Loco
                    ...
                    if (notifyXNetLocoFunc1)
                        notifyXNetLocoFunc1(SlaveRequestLocoInfo,
                                            XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata3]);
                    if (notifyXNetLocoFunc2)
                        notifyXNetLocoFunc2(SlaveRequestLocoInfo,
                                            XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata4]
                                              & 0x0F);
                    if (notifyXNetLocoFunc3)
                        notifyXNetLocoFunc3(SlaveRequestLocoInfo,
                                            XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetdata4]
                                              >> 4);
...
void XpressNetMasterClass::getNextXNetAdr(void)
{
    ...
    if (XNetAdr > 31)
    { // wenn letzte erreicht von Beginn!
    ...
    */
    ...
    CallByteInquiry =
      callByteParity((TempAdr % 32) | 0x40); // | 0x100;	// the address for Call Byte Window
    RequestAck = callByteParity(
      (TempAdr % 32) | 0x00); // | 0x100;		// the address for a request acknowlegement sent
    DirectedOps = callByteParity((TempAdr % 32)
                                 | 0x60); // | 0x100;		// the address when we are sending ops
    ...
    XNetRXBuffer.msg[XNetRXBuffer.put].data[XNetCallByte] =
      CallByteInquiry; // add CallByte to RX because we are Master
}
...
void XpressNetMasterClass::ReqLocoBusy(uint16_t Adr)
{
    ...
    {
        if (SlotLokUse[s] == Adr)
        { // if in use from X-Net device -> set busy
...
void XpressNetMasterClass::SetLocoInfo(uint8_t UserOps, uint8_t Steps, uint8_t Speed, uint8_t F0,
                                       uint8_t F1)
{
...
void XpressNetMasterClass::SetLocoInfoMM(uint8_t UserOps, uint8_t Steps, uint8_t Speed, uint8_t F0,
                                         uint8_t F1, uint8_t F2, uint8_t F3)
{
    ...
    /*ERROR: Steps change form > 99 to 28steps only!!! why?? (This info comes from
     * DCC:"notifyLokAll(..)"*/
    LocoInfo[4] |= F0;
    getXOR(LocoInfo, 10);
...
void XpressNetMasterClass::setFunc13to20(uint16_t Adr, uint8_t G4)
{
    ...
    if (Adr > 99) // Xpressnet long addresses (100 to 9999: AH/AL = 0xC064 to 0xE707)
        ...
        LocoInfoMM[3] =
          Adr >> 8; // short addresses (0 to 99: AH = 0x0000 and AL = 0x0000 to 0x0063)
...
void XpressNetMasterClass::SetBusy(uint8_t slot)
{
    for (byte s = 1; s < 32; s++)
    {
        if ((s != slot) && (SlotLokUse[s] == SlotLokUse[slot]))
...
void XpressNetMasterClass::setCVNack(void)
{
    XNetCVAdr = 0;
    XNetCVvalue = 0xFF;
    uint8_t sendStatus[] = {DirectedOps, 0x61, 0x13,
                            0x72}; // Programmierinfo �Daten nicht gefunden�
    XNetsend(sendStatus, 4);
}
...
{
    if (XNetSlaveMode != 0x00 && XNetTXBuffer.pos == 0)
    { // SLAVE MODE and Buffer is on the first byte
...
uint16_t XpressNetMasterClass::XNetReadBuffer()
{
    ...
    if (XNetTXBuffer.pos == 0x00)
    { // it is a CALLBYTE and we are MASTER!
        if (data != 0x00)
    ...
    if (XNetTXBuffer.pos >= XNetTXBuffer.msg[XNetTXBuffer.get].length)
...
#    endif
    ...
    { // If anything comes in Serial
        ...
        if (XNetSwSerial.readParity())
        { // detect parity bit set on the last message (MARK parity)
            XNetRXBuffer.msg[XNetRXBuffer.put].length = 0; // clear - only for sync!
...
#    endif
            ...
            XNetRXBuffer.msg[XNetRXBuffer.put].length++; // weitere Nachrichtendaten
            XNetRXBuffer.msg[XNetRXBuffer.put].data[XNetRXBuffer.msg[XNetRXBuffer.put].length] =
              data; // Zeichen aus UDR an Aufrufer zurueckgeben
        }
    }
    else
        return;
    ...
    if (XNetRXBuffer.msg[XNetRXBuffer.put].length >= 2)
    { // header and one data byte or more received
        // Check length - length is inside header but without header and xor!
        if (((XNetRXBuffer.msg[XNetRXBuffer.put].data[XNetheader] & 0x0F) + 2)
            == XNetRXBuffer.msg[XNetRXBuffer.put].length)
        { // reach defined data length + Header and CRC?
            /*
            #if defined (XNetDEBUG)
                    XNetSerial.print(XNetRXBuffer.put);
        ...
        if ((XNetRXBuffer.msg[XNetRXBuffer.put].length) >= XNetBufferMaxData)
        {                                  // overflow, without length byte!!!
